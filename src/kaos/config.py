...
class Prompts:
    ...
    COMPATIBILITY_ANALYZER = (
        ...
        "LƯU Ý QUAN TRỌNG VỀ PHƯƠNG PHÁP LÀM VIỆC ĐỂ TRÁNH BỊ TREO/TIMEOUT/CẠN KIỆT LƯỢT GỌI (BẮT BUỘC TUÂN THỦ):\n"
        "1. Bạn BẮT BUỘC phải đọc trực tiếp file JSON chứa database schema hiện hành tại {schema_path} bằng công cụ thích hợp. Mọi thông tin cần thiết về bảng, cột, quan hệ đều nằm trong file JSON này. Bạn không cần bất kỳ file nào khác ngoài file JSON này để phân tích schema hiện tại.\n"
        "2. TUYỆT ĐỐI KHÔNG ĐƯỢC DÙNG shell command (như 'find', 'tree', 'cat', 'grep', 'ls', 'python3', 'node'...) hoặc bất kỳ tool nào khác để quét codebase, tìm đọc code nguồn hoặc kiểm tra cấu trúc thư mục của target codebase. Việc sử dụng shell hay đọc code nguồn sẽ bị coi là vi phạm nghiêm trọng và làm cạn kiệt số lượt gọi tool (max-turns), dẫn đến timeout. Bạn chỉ được dùng tool để đọc đúng file {schema_path} và ghi file JSON kết quả.\n"
        "3. Nếu {raw_data_path} là 'Không cung cấp file database legacy (Chỉ phân tích nghiệp vụ spec).', bạn TUYỆT ĐỐI KHÔNG tìm đọc hay phân tích bất kỳ file Excel (.xlsx) hay cơ sở dữ liệu legacy nào khác.\n"
        "4. BẠN PHẢI TẠO FILE JSON KẾT QUẢ PHÂN TÍCH TẠI {output_json_path} NGAY LẬP TỨC. Bạn chỉ được thực hiện tối đa 2 turn: Turn 1 dùng tool để đọc {schema_path}, và Turn 2 dùng tool để viết/ghi file JSON kết quả tại {output_json_path}. Tuyệt đối không suy nghĩ lan man, không thực hiện thêm bất cứ hành động nào khác. LƯU Ý: Trong thuộc tính 'unified_diff' của file JSON kết quả, mọi ký tự đặc biệt hoặc dấu backslash (như \\n hoặc \\t hoặc \\) PHẢI được escape đúng chuẩn JSON (ví dụ sử dụng \\\\n hoặc escape tương ứng) để tránh gây ra lỗi cú pháp 'Invalid \\escape' khi parser đọc lại file JSON.\n\n"
