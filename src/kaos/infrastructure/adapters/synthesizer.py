...
class Synthesizer:
                ...
                if raw_type and sc_type and raw_type != sc_type:
                    conflicts.append(
                        ConflictPoint(
                            ...
                            suggestion="Chọn type phù hợp hoặc dùng conversion",
    ...
    def _detect_tenancy_issues(
        ...
        for table in schema.get("tables", []):
            ...
            has_org_id = any(c.get("name", "").lower() == "organization_id" for c in columns)
            if not has_org_id and table_name:
                conflicts.append(
                    ConflictPoint(
                        ...
                        suggestion="Thêm cột organization_id với kiểu UUID, foreign key đến organizations(id), NOT NULL",
                        location=table_name,
                        source="spec",
                    )
                )
    ...
    def _extract_file_actions(spec: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for i, req in enumerate(requirements[:20]):
            file_actions.append(
                {
                    "file": spec.get("target_module", f"spec_req_{i + 1}"),
                    "action": "implement",
                    "description": req[:200],
                }
            )
        ...
        if not file_actions:
            ...
            if desc and len(desc) > 10:
                file_actions.append(
                    {"file": spec.get("target_module", "module"), "action": "modify", "description": desc[:200]}
                )
